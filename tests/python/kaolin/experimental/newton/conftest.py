...
def simplicits_object(cube_mesh):
    ...
    yms = torch.full((NUM_SAMPLES,), float(SOFT_YOUNGS_MODULUS), device='cuda')
    prs = torch.full((NUM_SAMPLES,), float(POISSON_RATIO), device='cuda')
    rhos = torch.full((NUM_SAMPLES,), float(DENSITY), device='cuda')
...
def simplicits_model_with_object(simplicits_object):
    ...
    obj_idx = model.simplicits_scene.add_object(simplicits_object, num_qp=100)
...
def cantilever_beam_object():
    ...
    yms = 1e5 * torch.ones(pts.shape[0], device='cuda')
    prs = 0.45 * torch.ones(pts.shape[0], device='cuda')
    rhos = 500 * torch.ones(pts.shape[0], device='cuda')
    object_vol = (mesh.vertices.max(0)[0] - mesh.vertices.min(0)[0]).prod()

    return SimplicitsObject.create_rkpm(
